    ...
    public static class StringSimilarity
    {
        ...
        )
        {
            ...
            if (results.Count < maxSuggestions)
            {
                foreach (var (text, context) in candidates)
                {
                    ...
                    {
                        results.Add(
                            new MatchResult<T>(text, context, score, MatchType.PartialMatch)
                        );
                    }
                }
            }
            ...
            if (results.Count < maxSuggestions)
            {
                foreach (var (text, context) in candidates)
                {
                    var distance = StringDistance.GetLevenshteinDistance(
                        input,
                    ...
                    if (distance <= maxDistance)
                    {
                        ...
                        results.Add(
                            new MatchResult<T>(text, context, score, MatchType.Levenshtein)
                        );
                    }
                }
            }
        ...
        )
        {
            return FindSimilarWithContext(
                    input,
                    candidates.Select(c => (c, string.Empty)),
                    maxSuggestions
                )
                .Select(r => r.Text);
        }
            ...
            foreach (var inputWord in inputWords)
            {
                foreach (var candidateWord in candidateWords)
                {
                    ...
                    // Слово является префиксом
                        ...
                        candidateWord.StartsWith(inputWord, StringComparison.OrdinalIgnoreCase)
                    )
                    {
                    ...
                    // Слово содержится внутри
                        ...
                        candidateWord.IndexOf(inputWord, StringComparison.OrdinalIgnoreCase) >= 0
                    )
                    {
            ...
            for (i = 0; i < minLength; i++)
            {
                if (
                    !str1[str1.Length - 1 - i]
                        .ToString()
                        .Equals(str2[str2.Length - 1 - i].ToString(), comparison)
                )
                    break;
            }
